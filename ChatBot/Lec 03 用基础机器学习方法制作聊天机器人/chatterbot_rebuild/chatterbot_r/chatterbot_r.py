...
# import程序自定义的Adapter类：存储、输入、输出
from .storage import StorageAdapter
from .input import InputAdapter
from .output import OutputAdapter
from . import utils

class ChatBot_r(object):
    '''
    A conversational dialog chat bot.
    '''

    def __init__(self, name, **kwargs):
        # import 程序自定义的MultiLogicAdapter类，用于逻辑判断选择哪个Adapter模型
        ...
        # 下面这种kwargs.get(k1,k2)的方式，如k1存在，返回k1对应的value，如k1不存在，则返回k2.
        # 这样确保哪怕*_adapter调用时关键字错误，也有默认的adapter。
        ...
        utils.validata_adapter_class(input_adapter, InputAdapter)
        ...
        # Add the chatbot instance to each adapter to share information such as the name,
        # the current conversation, or other adapters
